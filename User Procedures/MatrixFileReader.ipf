...
Function execFunction (B_Struct) : ButtonControl
	...
	if(B_Struct.eventCode == 1)
		...
		elseif(cmpstr(B_Struct.ctrlName,"brickletDeployData") == 0)
	
			MFR_GetBrickletDeployData/R=(brickletID)
			if(V_flag != errorCode.SUCCESS)
				MFR_GetXOPErrorMessage
			endif
		...
		elseif(cmpstr(B_Struct.ctrlName,"reportBug") == 0)
			...
			string nb = UniqueName("reportBug",10,0)
			NewNoteBook/F=0/N=$nb/K=1 as "Report a MatrixfileReader Bug"
			MFR_GetReportTemplate 
			Notebook $nb, setData=S_value
...
Function myPanel()
	...
	SetVariable brickletID,win=$myPanel,title="Bricklet ID",value=brickletID,limits={1,inf,1},size={width,height},pos={oneBrickletHorPos+0.5*width,oneBrickletVertPos+heightOffset}
	...
	Button brickletDeployData,proc=execFunction,win=$myPanel,title="Get deploy data",size={width,height},pos={oneBrickletHorPos+0.5*width,oneBrickletVertPos+4*heightOffset}
	...
	SetVariable startBrickletID,win=$myPanel,title="Start Bricklet ID",value=startBrickletID,limits={1,inf,1},size={width,height},pos={rangeBrickletHorPos+0.5*width,rangeBrickletVertPos+heightOffset}
	SetVariable endBrickletID,win=$myPanel,title="End Bricklet ID",value=endBrickletID,limits={1,inf,1},size={width,height},pos={rangeBrickletHorPos+0.5*width,rangeBrickletVertPos+2*heightOffset}
	...
	TitleBox tb1,title="Thomas Braun (thomas dot braun at byte minus physics dot de)",pos={impressumHorPos+20,impressumVerPos},frame=0

End

Function d()
	print "#######################################################################################"
	print "#                            Commercial support for the MatrixFileReader XOP is now available! 	  			   #"
	print "#                            Don't hesitate to contact: info@byte-physics.de						  		   #"
	print "#######################################################################################"
End
